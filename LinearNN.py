...
def run_cv(x_dev, y_dev_arr, age_groups_arr, kfold, lr, senior_neg_weight, device, save_models=False):
    ...
    for _, (train_idx, val_idx) in enumerate(kfold.split(x_dev, y_dev_arr)):
        ...
        best_probs, _, state_dict = train_fold(
            model,
            make_loader(x_tr, y_tr, batch_size=64, shuffle=True, sample_weights=sample_weights),
            make_loader(x_val, y_val, batch_size=len(y_val), shuffle=False),
            y_val, num_epochs=300, lr=lr, weight_decay=1e-4, device=device
        )
    ...
    return oof_probs, fold_aucs, fold_models, fold_scalers


def ensemble_predict(fold_models, fold_scalers, x_test, input_dim, device):
    probs_list = []
    for state_dict, scaler in zip(fold_models, fold_scalers):
...
def main():
    ...
    senior_neg_weight_options = [1.0, 2.0, 3.0, 5.0]

    best_mean_auc = 0
    best_params = {'lr': 1e-3, 'senior_neg_weight': 1.0}

    print("=== Grid Search ===")
    for lr in lr_options:
        for snw in senior_neg_weight_options:
            oof_probs, fold_aucs, _, _ = run_cv(
                x_dev, y_dev_arr, age_groups_arr, kfold,
                lr, snw, device, save_models=False
            )
            mean_auc = np.mean(fold_aucs)
            print(f"lr={lr:.0e}  senior_neg_weight={snw:.1f}  CV AUC={mean_auc:.4f} ± {np.std(fold_aucs):.4f}")
            if mean_auc > best_mean_auc:
                best_mean_auc = mean_auc
                best_params = {'lr': lr, 'senior_neg_weight': snw}

    print(f"\nBest: lr={best_params['lr']:.0e}  senior_neg_weight={best_params['senior_neg_weight']:.1f}  CV AUC={best_mean_auc:.4f}")

    # Final CV run with best params — save fold models for ensemble
    print("\n=== Final CV run (saving fold models) ===")
    oof_probs, fold_aucs, fold_models, fold_scalers = run_cv(
        x_dev, y_dev_arr, age_groups_arr, kfold,
        best_params['lr'], best_params['senior_neg_weight'], device, save_models=True
    )
    ...
    test_probs = ensemble_predict(fold_models, fold_scalers, x_test, x_dev.shape[1], device)

    np.savetxt('yproba1_linearnn.txt', test_probs)
    print("Saved predictions to yproba1_linearnn.txt")
